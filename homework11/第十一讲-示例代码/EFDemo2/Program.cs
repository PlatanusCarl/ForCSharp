...
namespace EFDemo2 {
  class Program {
    static void Main(string[] args) {
      ...
      //添加帖子
      using (var context = new BloggingContext()) {
        var post = new Post() { Title = "Test3", Content = "Hell0", BlogId = newBlogId };
      ...
      //修改Id为newPostId的帖子的Comment
      using (var context = new BloggingContext()) {
        var post = new Post() { PostId = newPostId, Title = "Test3", Content = "Hell0", BlogId = newBlogId };
      ...
      //删除Id为blogId的博客及其帖子
      using (var context = new BloggingContext()) {
        var blog = context.Blogs.Include("Posts").FirstOrDefault(p => p.BlogId == newBlogId);
        if (blog != null) {
